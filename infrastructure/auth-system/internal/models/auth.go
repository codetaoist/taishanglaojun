...
type LoginRequest struct {
	...
	Remember  bool   `json:"remember"` // 是否记录器住登录状态
	UserAgent string `json:"-"`        // 从请求头获取
	IPAddress string `json:"-"`        // 从请求获取客户端IP地址
}

// Validate 自定义验证方法 - 支持邮箱或用户名登录
func (r *LoginRequest) Validate() error {
	// 检查是否提供了用户名或邮箱
	if r.Username == "" && r.Email == "" {
		return fmt.Errorf("用户名或邮箱不能为空")
	}
...
func (r *RegisterRequest) Validate() error {
	...
	if r.ConfirmPassword == "" {
		return fmt.Errorf("确认密码不能为空")
	}
	if r.Password != r.ConfirmPassword {
		return fmt.Errorf("密码和确认密码不匹配")
	}

	// 验证可选字段长度
	if len(r.FirstName) > 50 {
		return fmt.Errorf("名字长度不能超过50个字符")
	}
	if len(r.LastName) > 50 {
		return fmt.Errorf("姓氏长度不能超过50个字符")
	}
	if len(r.Phone) > 20 {
		return fmt.Errorf("电话号码长度不能超过20个字符")
	}
...
type RegisterResponse struct {
	User    *PublicUser `json:"user"`
	Message string      `json:"message"`
	Token   string      `json:"verification_token,omitempty"` // 邮箱件箱验证令牌
}
...
type LogoutRequest struct {
	...
	LogoutAll    bool      `json:"logout_all"` // 是否登录出所有有效会话
}

// LogoutResponse 登录出响应
type LogoutResponse struct {
	Message string `json:"message"`
}

// ForgotPasswordRequest 忘记录器密码请求
type ForgotPasswordRequest struct {
	Email string `json:"email" validate:"required,email"`
}

// ForgotPasswordResponse 忘记录器密码响应
...
// VerifyEmailRequest 验证邮箱件箱请求
type VerifyEmailRequest struct {
	Token string `json:"token" validate:"required"`
}

// VerifyEmailResponse 验证邮箱件箱响应
type VerifyEmailResponse struct {
	Message string `json:"message"`
}

// ResendVerificationRequest 重发验证邮箱件请求
type ResendVerificationRequest struct {
	Email string `json:"email" validate:"required,email"`
}

// ResendVerificationResponse 重发验证邮箱件响应
...
// UserPermission 用户权限关联表（用户于特殊权限分配）
