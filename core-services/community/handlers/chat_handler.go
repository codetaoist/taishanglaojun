...
// @Accept json
// @Produce json
// @Param request body CreateChatRoomRequest true "创建聊天室请?
...
func (h *ChatHandler) CreateChatRoom(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to create chat room", zap.Error(err), zap.String("user_id", userID.String()))
...
func (h *ChatHandler) GetChatRooms(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get chat rooms", zap.Error(err), zap.String("user_id", userID.String()))
		c.JSON(http.StatusInternalServerError, ErrorResponse{
...
func (h *ChatHandler) JoinChatRoom(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to join chat room", zap.Error(err), zap.String("user_id", userID.String()), zap.Uint("room_id", uint(roomID)))
		c.JSON(http.StatusInternalServerError, ErrorResponse{
...
// LeaveChatRoom 离开聊天房间
// @Summary 离开聊天房间
// @Description 用户离开指定聊天房间
// @Tags 聊天
// @Accept json
// @Produce json
// @Param room_id path int true "聊天室ID"
...
func (h *ChatHandler) LeaveChatRoom(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to leave chat room", zap.Error(err), zap.String("user_id", userID.String()), zap.Uint("room_id", uint(roomID)))
		c.JSON(http.StatusInternalServerError, ErrorResponse{
...
func (h *ChatHandler) GetChatMessages(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to get chat messages", zap.Error(err), zap.String("user_id", userID.String()), zap.Uint("room_id", uint(roomID)))
		c.JSON(http.StatusInternalServerError, ErrorResponse{
...
// @Produce json
// @Param room_id path int true "聊天室ID"
// @Param request body SendMessageRequest true "发送消息请求体"
...
func (h *ChatHandler) SendMessage(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Failed to send message", zap.Error(err), zap.String("user_id", userID.String()), zap.Uint("room_id", uint(roomID)))
		c.JSON(http.StatusInternalServerError, ErrorResponse{
