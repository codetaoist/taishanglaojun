...
type DeviceInfo struct {
	...
	Resolution   string    `json:"resolution"`    // 分辨?
	...
	Location     string    `json:"location"`      // 位置服务状?
	Permissions  []string  `json:"permissions"`   // 权限列表
	Capabilities []string  `json:"capabilities"`  // 设备能力
	LastActive   time.Time `json:"last_active"`   // 最后活跃时?
	IsOnline     bool      `json:"is_online"`     // 是否在线
}

// DevicePerformance 设备性能
type DevicePerformance struct {
	CPUUsage     float64   `json:"cpu_usage"`     // CPU使用?
	MemoryUsage  float64   `json:"memory_usage"`  // 内存使用?
	BatteryLife  int       `json:"battery_life"`  // 电池寿命
	NetworkSpeed float64   `json:"network_speed"` // 网络速度
	StorageUsed  float64   `json:"storage_used"`  // 存储使用?
...
type DeviceHealth struct {
	DeviceID         string    `json:"device_id"`         // 设备ID
	OverallHealth    string    `json:"overall_health"`    // 整体健康状?
	BatteryHealth    string    `json:"battery_health"`    // 电池健康状?
	...
	LastCheck        time.Time `json:"last_check"`        // 最后检查时?
}
