...
class BehaviorService {
  private basePath = '/user-behavior';
  ...
  async recordBehavior(request: Partial<BehaviorRequest>): Promise<void> {
    try {
      ...
      await apiClient.post(`${this.basePath}/record`, behaviorData);
    } catch (error) {
  ...
  async getUserProfile(userID?: string): Promise<UserProfile | null> {
    try {
      ...
      return (response.data as UserProfileResponse).data || null;
    } catch (error) {
  ...
  async getSimilarUsers(userID?: string, limit = 10): Promise<string[]> {
    try {
      ...
      const response = await apiClient.get(`${this.basePath}/similar-users`, {
        params: { 
      ...
      return (response.data as SimilarUsersResponse).data || [];
    } catch (error) {
