...
type QuantumGene struct {
	...
	Dominance       float64               `json:"dominance" db:"dominance"`         // 显性程度 0-1
	Stability       float64               `json:"stability" db:"stability"`         // 稳定性 0-1
	Mutability      float64               `json:"mutability" db:"mutability"`       // 可变性 0-1
	...
	ActivationLevel float64               `json:"activation_level" db:"activation_level"` // 激活水平
...
type GenePool struct {
	...
	PoolStats       GenePoolStats         `json:"pool_stats"`                       // 基因池统计
...
type GeneExpression struct {
	...
	Triggers        []ExpressionTrigger   `json:"triggers"`                               // 表达触发器
	Inhibitors      []ExpressionInhibitor `json:"inhibitors"`                             // 表达抑制器
	Context         ExpressionContext     `json:"context"`                                // 表达上下文
...
type GeneMutation struct {
	...
	MutationRate    float64               `json:"mutation_rate" db:"mutation_rate"`       // 突变率
...
type GenePrerequisite struct {
	...
	MinExpression   float64               `json:"min_expression"`            // 最小表达水平
	MaxExpression   float64               `json:"max_expression"`            // 最大表达水平
...
type GeneEffect struct {
	...
	IsPositive      bool                  `json:"is_positive"`               // 是否为正面效应
	IsPermanent     bool                  `json:"is_permanent"`              // 是否为永久效应
...
type GeneTrait struct {
	...
	Range           TraitRange            `json:"range"`                     // 取值范围
	Heritability    float64               `json:"heritability"`              // 遗传性 0-1
	Variability     float64               `json:"variability"`               // 可变性 0-1
	Metadata        map[string]interface{} `json:"metadata"`
}
...
type GenePoolStats struct {
	...
	DiversityIndex  float64               `json:"diversity_index"`           // 多样性指数
	StabilityIndex  float64               `json:"stability_index"`           // 稳定性指数
	EvolutionRate   float64               `json:"evolution_rate"`            // 进化速率
	MutationRate    float64               `json:"mutation_rate"`             // 突变率
	ExpressionLevel float64               `json:"expression_level"`          // 平均表达水平
	LastUpdated     time.Time             `json:"last_updated"`
}

// PoolEvolutionEvent 基因池进化事件
...
type MutationImpact struct {
	FunctionalChange float64               `json:"functional_change"`         // 功能变化 -1到1
	PerformanceChange float64             `json:"performance_change"`        // 性能变化 -1到1
	StabilityChange  float64              `json:"stability_change"`          // 稳定性变化 -1到1
	CompatibilityChange float64           `json:"compatibility_change"`      // 兼容性变化 -1到1
	OverallImpact    float64              `json:"overall_impact"`            // 总体影响 -1到1
...
type MutationCause struct {
	...
	IsExternal      bool                  `json:"is_external"`               // 是否为外部因素
	Metadata        map[string]interface{} `json:"metadata"`
}
...
const (
	...
	GeneCategoryAdaptability  GeneCategory = "adaptability"  // 适应性相关
	GeneCategoryTranscendence GeneCategory = "transcendence" // 超越性相关
	GeneCategoryStability     GeneCategory = "stability"     // 稳定性相关
...
const (
	MutationTypePoint       MutationType = "point"       // 点突变
...
const (
	...
	InteractionTypeAdditive    InteractionType = "additive"    // 加性作用
...
const (
	...
	TraitTypeCategorical TraitType = "categorical" // 分类型
	TraitTypeBoolean     TraitType = "boolean"     // 布尔型
	TraitTypeComplex     TraitType = "complex"     // 复合型
)
...
const (
	InhibitorTypeCompetitive    InhibitorType = "competitive"    // 竞争性抑制
	InhibitorTypeNoncompetitive InhibitorType = "noncompetitive" // 非竞争性抑制
...
func (gp *GenePool) GetDiversityScore() float64 {
	...
	typeDiv := float64(len(typeCount)) / 5.0  // 假设5种类型
	categoryDiv := float64(len(categoryCount)) / 8.0 // 假设8种类别
