...
// AutomatedKnowledgeGraphService 自动化知识图谱构建服�?
...
type AutomatedGraphConfig struct {
	...
	SemanticSimilarityWeight  float64 `json:"semantic_similarity_weight"`  // 语义相似度权�?
	StructuralSimilarityWeight float64 `json:"structural_similarity_weight"` // 结构相似度权�?
	ContentAnalysisDepth      int     `json:"content_analysis_depth"`      // 内容分析深度
	BatchProcessingSize       int     `json:"batch_processing_size"`       // 批处理大�?
	UpdateInterval            int     `json:"update_interval"`             // 更新间隔(分钟)
}

// AutomatedGraphCache 自动化图谱缓�?
...
// AutomatedGraphMetrics 自动化图谱指�?
type AutomatedGraphMetrics struct {
	NodesProcessed        int64     `json:"nodes_processed"`        // 已处理节点数
	RelationsInferred     int64     `json:"relations_inferred"`     // 推理出的关系�?
	SuccessfulInferences  int64     `json:"successful_inferences"`  // 成功推理�?
	FailedInferences      int64     `json:"failed_inferences"`      // 失败推理�?
	AverageConfidence     float64   `json:"average_confidence"`     // 平均置信�?
	ProcessingTime        int64     `json:"processing_time"`        // 处理时间(毫秒)
	LastProcessingTime    time.Time `json:"last_processing_time"`   // 最后处理时�?
...
type AutoBuildRequest struct {
	...
	CustomRules       []InferenceRule        `json:"custom_rules"`       // 自定义规�?
...
type KnowledgeGraphQualityMetrics struct {
	...
	Relevance         float64 `json:"relevance"`         // 相关�?
	Coverage          float64 `json:"coverage"`          // 覆盖�?
	Redundancy        float64 `json:"redundancy"`        // 冗余�?
	OverallScore      float64 `json:"overall_score"`     // 总体评分
}

// NewAutomatedKnowledgeGraphService 创建自动化知识图谱服�?
...
// AutoBuildFromContent 从内容自动构建知识图�?
...
// createNodeFromConcept 从概念创建知识节�?
...
// InferRelationsWithContext 带上下文的关系推�?
...
// calculateSimilarityMatrix 计算相似度矩�?
...
// calculateStructuralSimilarity 计算结构相似�?
...
// calculateTextSimilarity 计算文本相似�?
...
// validateAndOptimizeGraph 验证和优化图谱结�?
