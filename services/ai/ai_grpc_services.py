...
def serve_vector_service(host='0.0.0.0', port=50051, milvus_host='localhost', milvus_port='19530'):
    """Start the vector service gRPC server"""
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    
    # Create VectorServiceImpl with Milvus client
    vector_service = VectorServiceImpl(milvus_host=milvus_host, milvus_port=milvus_port)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='AI gRPC Services')
    parser.add_argument('--service', type=str, choices=['vector', 'model', 'both'], default='both',
                        help='Which service to start (vector, model, or both)')
    parser.add_argument('--vector-host', type=str, default='0.0.0.0',
                        help='Host for vector service (default: 0.0.0.0)')
    parser.add_argument('--vector-port', type=int, default=50051,
                        help='Port for vector service (default: 50051)')
    parser.add_argument('--model-host', type=str, default='0.0.0.0',
                        help='Host for model service (default: 0.0.0.0)')
    parser.add_argument('--model-port', type=int, default=50052,
                        help='Port for model service (default: 50052)')
    parser.add_argument('--milvus-host', type=str, default='localhost',
                        help='Host for Milvus server (default: localhost)')
    parser.add_argument('--milvus-port', type=str, default='19530',
                        help='Port for Milvus server (default: 19530)')
    
    args = parser.parse_args()
    
    servers = []
    
    if args.service in ['vector', 'both']:
        vector_server = serve_vector_service(
            host=args.vector_host, 
            port=args.vector_port,
            milvus_host=args.milvus_host,
            milvus_port=args.milvus_port
        )
        servers.append(vector_server)
    
    if args.service in ['model', 'both']:
        model_server = serve_model_service(
