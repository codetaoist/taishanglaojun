...
// PerformanceProfile 性能档案 (本地定义以替代domainServices.PerformanceProfile)
...
type CachedCrossModalMapping struct {
	...
	SourceModality ModalityType           `json:"source_modality"`
	TargetModality ModalityType           `json:"target_modality"`
...
type CrossModalAIRequest struct {
	...
	InputModalities     []ModalityType         `json:"input_modalities"`
	OutputModalities    []ModalityType         `json:"output_modalities"`
...
func (s *CrossModalAIIntegrationService) PerformCrossModalTranslation(
	...
	// 获取翻译模型
	model, err := s.getTranslationModel(sourceModality, targetModality)
	if err != nil {
...
func (s *CrossModalAIIntegrationService) validateRequest(request *CrossModalAIRequest) error {
	...
	for _, modality := range request.InputModalities {
		if !s.isModalitySupported(domainServices.ModalityType(modality)) {
