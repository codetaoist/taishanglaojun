  ...
  const getStudyTrendData = () => {
    const last30Days = Array.from({ length: 30 }, (_, i) => {
      ...
      const sessionsOnDate = studySessions.filter(session => 
        moment(session.date).isSame(date, 'day')
      );
            ...
            <Col xs={24} lg={12} key={goal.id}>
              <Card size="small">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <div>
                    <Text type="secondary">
                      目标日期: {moment(goal.targetDate).format('YYYY-MM-DD')}
                    </Text>
                  </div>
                          ...
                          <Space>
                            ...
                            {milestone.completedDate && (
                              <Text type="secondary" style={{ fontSize: '12px' }}>
                                ({moment(milestone.completedDate).format('MM-DD')})
                              </Text>
                            )}
                          </Space>
                        </div>
          ...
          {achievements.map(achievement => (
            <Col xs={24} sm={12} md={8} lg={6} key={achievement.id}>
              ...
              >
                ...
                <br />
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  {moment(achievement.earnedDate).format('YYYY-MM-DD')}
                </Text>
              </Card>
            </Col>
          ))}
          ...
          <div>
            <Space direction="vertical" style={{ width: '100%' }} size="large">
              ...
              <div>
                ...
                <div style={{ marginTop: '8px' }}>
                  <p><Text type="secondary">报名日期:</Text> {moment(selectedCourse.enrollDate).format('YYYY-MM-DD')}</p>
                  {selectedCourse.lastStudyDate && (
                    <p><Text type="secondary">最后学习:</Text> {moment(selectedCourse.lastStudyDate).format('YYYY-MM-DD')}</p>
                  )}
                  {selectedCourse.estimatedCompletion && (
                    <p><Text type="secondary">预计完成:</Text> {moment(selectedCourse.estimatedCompletion).format('YYYY-MM-DD')}</p>
                  )}
