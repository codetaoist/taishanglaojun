...
func (s *KnowledgeGraphAppService) GenerateLearningPath(ctx context.Context, req *LearningPathRequest) (*LearningPathResponse, error) {
	...
	// 构建路径推荐请求
	pathReq := &PathRecommendationRequest{
	...
	response := &LearningPathResponse{
		...
		TargetNodeID:     req.TargetNodeID,
		EstimatedTime:    personalizedPath.EstimatedDuration,
		Difficulty:       s.difficultyLevelToString(personalizedPath.Path.DifficultyLevel),
		RecommendedOrder: extractNodeIDs(personalizedPath.Path.Nodes),
		CreatedAt:        time.Now(),
	}

	// 获取路径中的节点和关系
	nodeIDs := extractNodeIDs(personalizedPath.Path.Nodes)
	nodes := make([]*NodeResponse, len(nodeIDs))
	for i, nodeID := range nodeIDs {
		node, err := s.graphRepo.GetNode(ctx, s.defaultGraphID, nodeID)
		if err != nil {
			continue // 跳过无法获取的节点
		}
		nodes[i] = s.buildNodeResponse(node)
	}
	response.Nodes = nodes

	// 构建里程碑
	milestones := make([]*PathMilestone, len(personalizedPath.Milestones))
	for i, milestone := range personalizedPath.Milestones {
		milestones[i] = &PathMilestone{
			ID:          uuid.New(), // 生成新的ID
			NodeID:      milestone.NodeID,
			Name:        fmt.Sprintf("Milestone %d", milestone.Position+1),
			Description: fmt.Sprintf("Learning milestone at position %d", milestone.Position),
			Order:       milestone.Position,
			IsRequired:  true, // 默认为必需
		}
	}
	response.Milestones = milestones

	// 构建适应建议
	adaptations := make([]*PathAdaptation, len(personalizedPath.Adaptations))
	for i, adaptation := range personalizedPath.Adaptations {
		adaptations[i] = &PathAdaptation{
			ID:         uuid.New(), // 生成新的ID
			Type:       adaptation.Type,
			Reason:     adaptation.Description,
			Suggestion: fmt.Sprintf("建议调整: %s (影响度: %.2f, 置信度: %.2f)", adaptation.Description, adaptation.Impact, adaptation.Confidence),
			Priority:   int(adaptation.Impact * 10), // 将影响度转换为优先级
		}
	}
...
// AnalyzeGraph 分析图谱
func (s *KnowledgeGraphAppService) AnalyzeGraph(ctx context.Context, req *GraphAnalysisRequest) (interface{}, error) {
...
func (s *KnowledgeGraphAppService) analyzeGraphStructure(ctx context.Context, req *GraphAnalysisRequest) (*GraphAnalysisResult, error) {
	analysisReq := &GraphAnalysisRequest{
		GraphID:      s.defaultGraphID,
...
func (s *KnowledgeGraphAppService) analyzeLearningGaps(ctx context.Context, req *GraphAnalysisRequest) ([]*LearningGap, error) {
	if req.LearnerID == nil {
	...
	// 调用领域服务的方法来识别学习缺口
	// 创建领域服务的分析请求
	domainReq := &GraphAnalysisRequest{
		GraphID:      req.GraphID,
	...
	if domainResult != nil {
		...
		for i := 0; i < 3; i++ {
			gap := &LearningGap{
				...
				Severity:    "medium",
				Suggestions: []string{"Review prerequisite concepts", "Practice more exercises"},
			}
...
func (s *KnowledgeGraphAppService) analyzeOptimization(ctx context.Context, req *GraphAnalysisRequest) ([]*OptimizationSuggestion, error) {
	// 获取知识图谱
	_, err := s.graphRepo.GetGraph(ctx, req.GraphID)
	if err != nil {
...
func (s *KnowledgeGraphAppService) generateRecommendations(ctx context.Context, req *GraphAnalysisRequest) ([]*ConceptRecommendation, error) {
	if req.LearnerID == nil {
	...
	for i := 0; i < 5; i++ {
		recommendation := &ConceptRecommendation{
			ConceptID:   uuid.New(),
			Title:       fmt.Sprintf("Recommended Concept %d", i+1),
			Description: fmt.Sprintf("This is a recommended concept for learning"),
			Relevance:   0.8,
		}
