...
本架构遵循页面提出的多端适配与插件化策略：B/S 为主、C/S 辅助，并以 CI/CD 贯穿插件生命周期。调整为各终端使用原生语言，Web 使用 React。[0]
...
- 后端：Go（Gin），模块地址 `github.com/codetaoist/taishanglaojun`，OpenAPI 3.0 为事实源。[0]
...
- 核心服务层：`laojun`（插件/审计/配置/设备）与 `taishang`（模型/向量/任务/多模态）。
...
graph TD
    ...
    接入层 --> laojun[laojun：基础底座]
    接入层 --> taishang[taishang：AI能力]
...
## 模块映射（laojun/taishang）
- laojun：用户与权限、设备管理、插件注册中心、审计与配置、网关与适配。
- taishang：大模型集成、意图识别、多模态转换、任务编排、向量数据库管理、容错与优化。
...
sequenceDiagram
  ...
  participant GW as 网关
  participant Tai as taishang
  participant Lao as laojun

  Mobile->>GW: 语音指令（生成巡检脚本）
  GW->>Tai: 请求转发（鉴权）
  Tai->>Tai: 意图识别+调用AI插件生成脚本
  Tai->>Lao: 脚本存储与审计
...
## 接口边界与治理
- 路由分域：`/api/laojun/*` 与 `/api/taishang/*`；前缀与权限策略对齐。
