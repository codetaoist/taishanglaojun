...
class ChatViewModel: ObservableObject {
    ...
    // MARK: - Multimodal Message Support
    ...
    func sendImageMessage(_ imageData: Data) {
        ...
        // 保存图片到本地
        if let imageURL = MediaService.shared.saveImageToLocal(imageData) {
            let content = imageURL.absoluteString
            ...
            isSending = false
            handleError(NSError(domain: "ChatViewModel", code: -1, userInfo: [NSLocalizedDescriptionKey: "保存图片失败"]))
        }
    }
    
    /// 发送音频消息
    func sendAudioMessage(_ audioURL: URL) {
        guard let conversationId = currentConversation?.id else { return }
        
        isSending = true
        
        let content = audioURL.absoluteString
