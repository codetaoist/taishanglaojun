...
func main() {
	...
	log.Println("健康管理服务 (Mock版本) 启动在端口 8081")
...
func getHealthData(c *gin.Context) {
	...
	if !exists {
		c.JSON(404, gin.H{"error": "健康数据不存在"})
		return
	}
	c.JSON(200, data)
}
...
func getHealthProfile(c *gin.Context) {
	...
	if !exists {
		c.JSON(404, gin.H{"error": "健康档案不存在"})
		return
	}
	c.JSON(200, profile)
}
	...
	insights := []map[string]interface{}{
		{
			"type":        "positive",
			"title":       "运动表现优秀",
			"description": "本周平均步数达到9000步，超过目标值",
			"priority":    "medium",
		},
		{
			"type":        "warning",
			"title":       "睡眠不足",
			"description": "最近几天睡眠时间不足7小时，建议调整作息",
			"priority":    "high",
		},
		{
			"type":        "info",
			"title":       "心率稳定",
			"description": "心率保持在正常范围内，继续保持",
			"priority":    "low",
		},
	}
...
func getHealthRecommendations(c *gin.Context) {
	...
	recommendations := []map[string]interface{}{
		...
		},
		{
			"category":    "diet",
			"title":       "控制盐分摄入",
			"description": "减少高盐食物摄入，有助于控制血压",
			"priority":    "medium",
		},
		{
			"category":    "sleep",
			"title":       "改善睡眠质量",
			"description": "建立规律的睡眠时间，避免睡前使用电子设备",
			"priority":    "high",
		},
	}
	...
	alerts := []map[string]interface{}{
		{
			...
			"type":        "warning",
			"title":       "血压异常",
			"description": "您的血压读数为140/90，建议咨询医生",
			"severity":    "high",
		...
		},
		{
			...
			"type":        "info",
			"title":       "运动目标达成",
			"description": "恭喜！您今天已完成10000步的运动目标",
			"severity":    "low",
