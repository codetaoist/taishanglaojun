...
func (s *LearnerService) GetLearningAnalytics(ctx context.Context, learnerID uuid.UUID, timeRange string) (*LearningAnalyticsReport, error) {
	// 构建分析请求
	...
	req := &AnalyticsRequest{
		LearnerID:         learnerID,
		TimeRange:         AnalyticsTimeRange{StartDate: startTime, EndDate: endTime},
...
func (s *LearnerService) GetPersonalizedRecommendations(ctx context.Context, learnerID uuid.UUID) (*PersonalizedRecommendations, error) {
	...
	// 获取学习路径推荐
	pathRecommendations, err := s.pathService.RecommendPersonalizedPaths(ctx, &PathRecommendationRequest{
		LearnerID:    learnerID,
		GraphID:      uuid.New(), // 使用默认图谱ID
		TargetNodeID: uuid.New(), // 使用默认目标节点
		MaxPaths:     5,
	})
	if err != nil {
	...
	conceptReq := &ConceptRecommendationRequest{
		GraphID:            uuid.New(), // 使用默认图谱ID
		LearnerID:          learnerID,
	...
	return &PersonalizedRecommendations{
		...
		ConceptRecommendations: conceptRecommendations,
...
func (s *LearnerService) updateLearningPathRecommendations(ctx context.Context, learner *entities.Learner) {
	...
	go func() {
		...
		_, err := s.pathService.RecommendPersonalizedPaths(ctx, &PathRecommendationRequest{
...
func (s *LearnerService) updateLearningAnalytics(ctx context.Context, learnerID uuid.UUID) {
	...
	req := &AnalyticsRequest{
		LearnerID:         learnerID,
		TimeRange:         AnalyticsTimeRange{StartDate: time.Now().AddDate(0, -1, 0), EndDate: time.Now()},
		AnalysisType:      "comprehensive",
		Granularity:       "daily",
		IncludeComparison: true,
