...
func (sm *ServiceManager) GetHealthStatus() map[string]interface{} {
	...
	// 获取服务健康状态
	serviceHealth := integration.GetHealthStatus()
	health["services"] = serviceHealth
	
	// 检查是否有不健康的服务
	for serviceName, serviceStatus := range serviceHealth {
		if statusMap, ok := serviceStatus.(map[string]interface{}); ok {
...
func (sm *ServiceManager) GetMetrics() map[string]interface{} {
	...
	if sm.running {
		integration, err := sm.initializer.GetIntegration()
		if err == nil {
			serviceMetrics := integration.GetMetrics()
...
func (sm *ServiceManager) ProcessRequest(ctx context.Context, request interface{}) (interface{}, error) {
	...
	retryErr := sm.errorHandler.RetryOperation(ctx, func() error {
		...
		result, processErr = integration.ProcessLearningRequest(ctx, request)
		return processErr
