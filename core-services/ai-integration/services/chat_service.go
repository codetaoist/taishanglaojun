...
func (s *ChatService) Chat(ctx context.Context, req *ChatRequest) (*ChatResponse, error) {
	...
	// 获取或创建会�?	conversation, err := s.getOrCreateConversation(ctx, req.UserID, req.SessionID)
	if err != nil {
		return nil, fmt.Errorf("failed to get conversation: %w", err)
	}

	// 构建提供者请�?	providerReq := s.buildProviderRequest(req, conversation)

	// 调用AI提供�?	providerResp, err := s.provider.Chat(ctx, providerReq)
	if err != nil {
		return nil, fmt.Errorf("provider error: %w", err)
	}

	// 保存消息到会�?	if err := s.saveMessages(ctx, conversation.ID, req.Message, providerResp.Message); err != nil {
...
// ListConversations 获取用户的对话列�?func (s *ChatService) ListConversations(ctx context.Context, userID string, page, size int) ([]*models.ConversationSummary, error) {
...
// getOrCreateConversation 获取或创建会�?func (s *ChatService) getOrCreateConversation(ctx context.Context, userID, sessionID string) (*models.Conversation, error) {
	if sessionID != "" {
	...
	// 创建新会�?	conversation := &models.Conversation{
...
// buildProviderRequest 构建提供者请�?func (s *ChatService) buildProviderRequest(req *ChatRequest, conv *models.Conversation) providers.ChatRequest {
