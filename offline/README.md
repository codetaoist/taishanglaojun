...
码道CLI工具是一个轻量级的跨平台命令行程序，通过自然语言交互为开发者提供AI驱动的编程辅助。无论是代码生成、问题解答还是项目管理，只需一句话即可完成复杂的开发任务。
...
# Linux/macOS: sudo cp lo /usr/local/bin/
```
...
码道CLI遵循"自然语言优先"的设计理念：
- **直观表达** - 用自然语言描述需求，无需记忆复杂命令
- **智能理解** - AI自动理解上下文，提供精准建议
- **安全执行** - 重要操作需确认，支持预览模式
...
|----------|------|----------|
| **认证** | `lo login` | 登录码道平台 |
| **项目** | `lo project create/list/info` | 项目管理 |
| **AI交互** | `lo ai "指令"` | AI智能编程 |
| **知识库** | `lo ask "问题"` | 查询项目知识 |
| **配置** | `lo config get/set` | 配置管理 |
| **工具** | `lo status/history/version` | 实用工具 |
...
**代码生成**
```bash
lo ai "用Go写一个HTTP服务器，包含路由和中间件"
lo ai "创建一个用户注册的API接口，包含参数验证"
lo ai "生成一个Redis缓存工具类"
```
