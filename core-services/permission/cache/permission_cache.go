...
func (c *RedisPermissionCache) SetUserPermissions(ctx context.Context, userID, tenantID string, permissions []*permission.Permission, ttl time.Duration) error {
	key := c.buildUserPermissionsKey(userID, tenantID)
	...
	if err != nil {
		c.logger.Error("Failed to set user permissions cache", zap.String("key", key), zap.Error(err))
		return &CacheError{Operation: "set", Key: key, Err: err}
	}

	c.logger.Debug("User permissions cached", zap.String("user_id", userID), zap.String("tenant_id", tenantID), zap.String("key", key))
	return nil
}

// GetUserPermissions 获取用户权限缓存
func (c *RedisPermissionCache) GetUserPermissions(ctx context.Context, userID, tenantID string) ([]*permission.Permission, error) {
	key := c.buildUserPermissionsKey(userID, tenantID)
	data, err := c.client.Get(ctx, key).Result()
	if err != nil {
		if err == redis.Nil {
		...
		c.logger.Error("Failed to get user permissions cache", zap.String("key", key), zap.Error(err))
		return nil, &CacheError{Operation: "get", Key: key, Err: err}
	}
	...
	if err != nil {
		c.logger.Error("Failed to deserialize user permissions", zap.String("key", key), zap.Error(err))
		return nil, &CacheError{Operation: "deserialize", Key: key, Err: err}
	}

	c.logger.Debug("User permissions cache hit", zap.String("user_id", userID), zap.String("tenant_id", tenantID), zap.String("key", key))
	return permissions, nil
}

// DeleteUserPermissions 删除用户权限缓存
func (c *RedisPermissionCache) DeleteUserPermissions(ctx context.Context, userID, tenantID string) error {
	key := c.buildUserPermissionsKey(userID, tenantID)
	err := c.client.Del(ctx, key).Err()
	if err != nil {
		c.logger.Error("Failed to delete user permissions cache", zap.String("key", key), zap.Error(err))
		return &CacheError{Operation: "delete", Key: key, Err: err}
	}

	c.logger.Debug("User permissions cache deleted", zap.String("user_id", userID), zap.String("tenant_id", tenantID), zap.String("key", key))
	return nil
}

// SetUserRoles 设置用户角色缓存
func (c *RedisPermissionCache) SetUserRoles(ctx context.Context, userID string, tenantID string, roles []*permission.Role, ttl time.Duration) error {
...
func (c *RedisPermissionCache) SetPermissionCheck(ctx context.Context, checkKey string, result *permission.PermissionCheckResponse, ttl time.Duration) error {
...
// GetPermissionCheck 获取权限检查结果缓存
func (c *RedisPermissionCache) GetPermissionCheck(ctx context.Context, checkKey string) (*permission.PermissionCheckResponse, error) {
	key := c.buildCheckResultKey(checkKey)
	data, err := c.client.Get(ctx, key).Result()
	if err != nil {
		if err == redis.Nil {
	...
	var result permission.PermissionCheckResponse
...
// DeletePermissionCheck 删除权限检查结果缓存
func (c *RedisPermissionCache) DeletePermissionCheck(ctx context.Context, checkKey string) error {
	key := c.buildCheckResultKey(checkKey)
	err := c.client.Del(ctx, key).Err()
	if err != nil {
...
// ClearUserCache 清空用户相关缓存
func (c *RedisPermissionCache) ClearUserCache(ctx context.Context, userID, tenantID string) error {
	return c.InvalidateUserCache(ctx, userID, tenantID)
}
...
// 构建用户角色缓存?
func (c *RedisPermissionCache) buildUserPermissionsKey(userID, tenantID string) string {
	return fmt.Sprintf("%s:user_permissions:%s:%s", c.config.KeyPrefix, userID, tenantID)
}

func (c *RedisPermissionCache) buildUserRolesKey(userID, tenantID string) string {
	return fmt.Sprintf("%s:user_roles:%s:%s", c.config.KeyPrefix, userID, tenantID)
}
