...
func (s *MultimodalRecommendationService) GetMultimodalRecommendations(
	...
	// 获取学习者信息
	learner, err := s.learnerRepo.GetByUserID(ctx, req.UserID)
	if err != nil {
...
func (s *MultimodalRecommendationService) getCandidateContent(
	...
	filters := map[string]interface{}{
		"difficulty_levels": s.getAllowedDifficultyLevels(req.MaxDifficulty, learner),
		"content_types":     req.PreferredTypes,
		"exclude_completed": true,
		"learner_id":        learner.ID,
	}

	if req.TimeAvailable != nil {
		filters["max_duration"] = *req.TimeAvailable
	}

	return s.contentRepo.FindByFilters(ctx, filters)
}
