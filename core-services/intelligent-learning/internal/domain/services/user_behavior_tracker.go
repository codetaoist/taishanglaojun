...
// NewUserBehaviorTracker 创建用户行为跟踪器
...
func (ubt *UserBehaviorTracker) analyzeTimePatterns(events []*BehaviorEvent) *LearningPattern {
	...
	if peakHour >= 6 && peakHour < 12 {
		timeDescription = "早晨学习者"
	} else if peakHour >= 12 && peakHour < 18 {
		timeDescription = "下午学习者"
	} else {
		timeDescription = "晚间学习者"
	}
	...
	return &LearningPattern{
		...
		Evidence:    []string{fmt.Sprintf("最活跃时间段: %d:00", peakHour)},
		Impact:      "影响学习效率和专注度",
	}
}
...
func (ubt *UserBehaviorTracker) analyzeContentPatterns(events []*BehaviorEvent) *LearningPattern {
	...
	return &LearningPattern{
		...
		Confidence:  float64(maxCount) / float64(len(events)),
		Evidence:    []string{fmt.Sprintf("最常使用内容类型: %s", preferredType)},
		Impact:      "影响学习参与度和理解效果",
	}
}
...
func (ubt *UserBehaviorTracker) analyzePacePatterns(events []*BehaviorEvent) *LearningPattern {
	...
	if avgDuration < 15*time.Minute {
		paceDescription = "快节奏学习者"
	} else if avgDuration < 45*time.Minute {
		paceDescription = "中等节奏学习者"
	} else {
		paceDescription = "深度学习者"
	}
	...
	if len(events) > 0 {
		...
		if avgDuration < 10*time.Minute {
			insights = append(insights, EngagementInsight{
				Type:        "session_length",
				Title:       "会话时间较短",
				Description: "平均学习会话时间较短，可能影响深度学习",
				Severity:    "medium",
				Impact:      0.6,
	...
	if len(events) > 0 {
		...
		if frequency < 1 {
			actions = append(actions, BehaviorRecommendedAction{
				...
				Type:           "frequency",
				Title:          "增加学习频率",
				Description:    "建议增加学习频率以提高学习效果",
				Priority:       2,
				ExpectedImpact: 0.7,
				Timeline:       "1-2周",
				Implementation: []string{"设置每日学习提醒", "制定学习计划", "设定小目标"},
				SuccessMetrics: []string{"学习天数增加", "完成率提升"},
			})
		}
	}
	...
	if len(events) >= 10 {
		...
		if recentEngagement > 0.6 {
			insights = append(insights, PredictiveInsight{
				...
				Confidence:     0.75,
				TimeHorizon:    7 * 24 * time.Hour, // 7天
				Factors:        []string{"持续的学习习惯", "高参与度会话"},
				Recommendations: []string{"保持当前学习节奏", "适当增加挑战性内容"},
			})
		} else {
			insights = append(insights, PredictiveInsight{
				Type:           "engagement_risk",
				Prediction:     "学习参与度可能下降",
				Confidence:     0.65,
				TimeHorizon:    3 * 24 * time.Hour, // 3天
				Factors:        []string{"最近会话时间较短", "参与度下降趋势"},
				Recommendations: []string{"调整学习内容", "增加互动元素", "设置激励机制"},
			})
		}
	}
