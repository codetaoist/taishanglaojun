...
// DynamicDatabase 动态数量据库管理器器
type DynamicDatabase struct {
	databases map[string]*Database // 数量据库连接池
	current   string               // 当前使用户的数量据库
	mutex     sync.RWMutex         // 读写锁
	logger    *zap.Logger
}

// DatabaseSwitchConfig 数量据库切换配置
type DatabaseSwitchConfig struct {
	Name     string                `json:"name"`     // 数量据库配置名
	Database config.DatabaseConfig `json:"database"` // 数量据库配置
	Redis    config.RedisConfig    `json:"redis"`    // Redis配置
}

// NewDynamicDatabase 创建动态数量据库管理器
...
// GetCurrentDatabaseName 获取当前数量据库配置名
