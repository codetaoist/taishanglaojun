...
// CoordinateWithConfidence 带置信度的三轴坐标
...
type CoordinationRequest struct {
	...
	Priority     int                    `json:"priority" default:"1"` // 优先级1-5
}
...
type Constraint struct {
	...
	Operator    string      `json:"operator"`    // 操作符(eq, gt, lt, gte, lte)
	Value       interface{} `json:"value"`       // 约束值
	Description string      `json:"description"` // 约束描述
}

// SequenceResult S轴能力序列结果
...
type CoordinateAnalysis struct {
	InputComplexity       float64                `json:"input_complexity"`       // 输入复杂度
	TechnicalRequirement  float64                `json:"technical_requirement"`  // 技术要求
